                       ...
                       ICM426XX_APEX_CONFIG4_TILT_WAIT_TIME_t tilt_wait_time)
{
	...
	rc |= inv_icm426xx_set_apex_frequency(&icm_driver, tilt_freq);

	/* Enable the tilt */
	rc |= inv_icm426xx_enable_apex_tilt(&icm_driver);
...
int GetDataFromInvDevice(void)
{
	...
	if (int_status3 & (BIT_INT_STATUS3_TILT_DET))
		INV_MSG(INV_MSG_LEVEL_INFO, "%u: Tilt event detected", irq_timestamp);
