...
void GH3X2X_AlgoVersion(GU8 uchFunctionID, GCHAR version[100])
{
    ...
    case GH3X2X_FUNC_OFFSET_ADT:
        memcpy(version, GH3x2xAdtVersion(), strlen((const GCHAR *)GH3x2xAdtVersion()) + 1);
        break;
#endif
#if __USE_GOODIX_HR_ALGORITHM__
    ...
    default:
		...
		GH3X2X_SAMPLE_ALGO_LOG_PARAM("version = %s\n", version);
}
...
GS8 GH3X2X_AlgoInit(GU32 unFunctionID)
{
    ...
    for(GU8 uchFunCnt = 0; uchFunCnt < GH3X2X_FUNC_OFFSET_MAX; uchFunCnt ++)
    {
        if (((1<<uchFunCnt) & unFunctionID) == (1<<uchFunCnt))
        {
            ...
            {
                GH3X2X_ALGO_LOG_PARAM("[%s]:Function(function id = %d) already init!\r\n", __FUNCTION__, (int)uchFunCnt);
            }
            else
            {
                if (g_pfnAlgorithmCallFunc[uchFunCnt][GH3X2X_ALGO_INIT_FUNCTION] != GH3X2X_PTR_NULL)
                {
                    ...
                    {
                        GH3X2X_ALGO_LOG_PARAM("[%s]:Function(function id = %d) init fail! errcode = %d\r\n", __FUNCTION__, (int)uchFunCnt, (int)chRet);
                    }
                    else
                    {
                        GH3X2X_ALGO_LOG_PARAM("[%s]:Function(function id = %d) init success!\r\n", __FUNCTION__, (int)uchFunCnt);
                        g_uchAlgInitFlag[uchFunCnt] = 1;
                    }
                }
                else
                {
                    GH3X2X_ALGO_LOG_PARAM("[%s]:Function(function id = %d) init func null!\r\n", __FUNCTION__, (int)uchFunCnt);
                }
...
GS8 GH3X2X_AlgoDeinit(GU32 unFunctionID)
{
    ...
    for(GU8 uchFunCnt = 0; uchFunCnt < GH3X2X_FUNC_OFFSET_MAX; uchFunCnt ++)
    {
        if (((1<<uchFunCnt) & unFunctionID) == (1<<uchFunCnt))
        {
            ...
            {
                GH3X2X_ALGO_LOG_PARAM("[%s]:Function(function id = %d) didn't init!\r\n", __FUNCTION__, (int)uchFunCnt);
            }
            else
            {
                if (g_pfnAlgorithmCallFunc[uchFunCnt][GH3X2X_ALGO_DEINIT_FUNCTION] != GH3X2X_PTR_NULL)
                {
                    ...
                    if (chRet != GH3X2X_RET_OK)
                    {
                        GH3X2X_ALGO_LOG_PARAM("[%s]:Function(function id = %d) deinit fail! errcode = %d\r\n", __FUNCTION__, (int)uchFunCnt, (int)chRet);
                    }
                    else
                    {
                        GH3X2X_ALGO_LOG_PARAM("[%s]:Function(function id = %d) deinit success!\r\n", __FUNCTION__, (int)uchFunCnt);
                        g_uchAlgInitFlag[uchFunCnt] = 0;
                    }
                }
                else
                {
                    GH3X2X_ALGO_LOG_PARAM("[%s]:Function(function id = %d) deinit func null!\r\n", __FUNCTION__, (int)uchFunCnt);
                }
            }
            GH3X2X_RET_ERROR_CHECK(chRet);
        }
    ...
    for(GU8 uchFunCnt = 0; uchFunCnt < GH3X2X_FUNC_OFFSET_MAX; uchFunCnt ++)
    {
        if (((1<<uchFunCnt) & unFunctionID) == (1<<uchFunCnt))
        {
            ...
            if (GH3X2X_CHECK_BIT_SET(g_unAlgoFuncStartedBitmap, unFunctionID))
            {
                ...
                if (0 == g_uchAlgInitFlag[uchFunCnt])
                {
                    //GH3X2X_ALGO_LOG_PARAM("Function(funcition id = %d) didn't init!\r\n",(int)uchFunCnt);
                }
                else
                {
