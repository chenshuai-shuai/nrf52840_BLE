...
#define SPI_CS_DEV   DT_LABEL(DT_NODELABEL(gpio0))
#define SPI_CS_PIN   (02)

#define SPI_INT_DEV  DT_LABEL(DT_NODELABEL(gpio0))
#define SPI_INT_PIN  (24)

#define SPI_RST_DEV  DT_LABEL(DT_NODELABEL(gpio0))
#define SPI_RST_PIN  (25)
...
{
    /* init spi and cs pin */
    gh3x2x_info.spi_dev = device_get_binding(SPI_DEV);
    if(NULL == gh3x2x_info.spi_dev)
    {
    ...
    gh3x2x_info.spi_cs_dev = device_get_binding(SPI_CS_DEV);
    if(NULL == gh3x2x_info.spi_cs_dev)
    {
        printk("Failed to find device %s\n", SPI_CS_DEV);
        return;
    }
...
{
    gh3x2x_info.spi_rst_dev = device_get_binding(SPI_RST_DEV);
    if(NULL == gh3x2x_info.spi_rst_dev)
    {
        printk("Failed to find device %s\n", SPI_RST_DEV);
        return;
    }
...
{
    gh3x2x_info.spi_int_dev = device_get_binding(SPI_INT_DEV);
    if(NULL == gh3x2x_info.spi_int_dev)
    {
        printk("Failed to find device %s\n", SPI_INT_DEV);
        return;
    }
...
{
    (*pPrintfUser) = printk;   //use printf in <stdio.h>  or use equivalent function in your platform
